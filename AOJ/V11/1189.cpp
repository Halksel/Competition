...
//     REP(i,0,u+2){
//       REP(j,0,u+2){
//         scout(3) << tmp[cave[i][j]];
//       }
//       cout << endl;
//     }
    int cdx[] = {-1,0,1},ca = arr[cave[cx][cy]] == 1 ? cave[cx][cy] : 0;
    dp[cx][cy] = tmp[cave[cx][cy]];
    pii ans(arr[cave[cx][cy]],ca);
    REP(i,cx+1,u+2){
      REP(j,0,u+2){
        if(cave[i][j] != 0 && cave[i][j] <= m){
          int ma = 0;
          rep(k,3){
            int X = i-1,Y = j+cdx[k];
            if(value(X,Y,m,m) && cave[X][Y] <= m){
              if(ma < tmp[cave[i][j]]+dp[X][Y]){
                ma = max(ma,tmp[cave[i][j]] + dp[X][Y]);
                ans.fi = dp[X][Y];
                if(arr[cave[i][j]]){
                  ans.se = cave[i][j];
                }
    ...
    rep(i,u+2){
      if(dp[u+2-1][i] == ans.fi){
        if(ans.se > cave[u+2-1][i] && arr[cave[u+2-1][i]]){
          ans.se = cave[u+2-1][i];
        }
      }
    }
